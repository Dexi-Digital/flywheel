...
async function fetchFernanda(sb: SupabaseClient) {
  const { data, error } = await sb
    .from('leads_nao_convertidos_fase02')
    .select('id,created_at,nome,veiculo,intencao,raciocinio_ai,ultima_interacao,valor_potencial,email,telefone')
...
export const fernandaService: AgentService = {
  ...
  async getAgent(agentId: string) {
    ...
    const rows = await fetchFernanda(sb);
